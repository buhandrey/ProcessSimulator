...
public:
    ...
    double stat_get_mean () const { return stat_M1_x; }
...
void statistics::update_stat (double x) {
    stat_counter++;
...
double statistics::stat_get_disp_root () const {
    if (stat_counter<=1) return NAN;
    return sqrt(stat_M2_x/(stat_counter-1.0));
}

double statistics::stat_get_disp () const {
    if (stat_counter<=1) return NAN;
    return stat_M2_x/(stat_counter-1.0);
}

double statistics::stat_get_skew () const {
...
double statistics::stat_get_kurt () const {
