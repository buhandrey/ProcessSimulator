...
public:
    ...
    double extreme_part_outside (double sigma_out) const { return 1.0 - extreme_part_inside(sigma_out); }
    long get_bins_number () { return prob_filled; }
};
...
void distribution::resize (long distr_size_manual) {
...
int distribution::calc_probability (double x) {
...
double distribution::get_prob_dens (long i) const {
    if ((i<distr_size)&&(i>=0)) {
...
double distribution::get_prob_left (long i) const {
    if ((i<distr_size)&&(i>=0)) {
        return prob_mins[i];
    }
    else
...
double distribution::get_prob_right (long i) const {
    if ((i<distr_size)&&(i>=0)) {
        return prob_maxs[i];
    }
    else
...
double distribution::get_prob_mean (long i) const {
    if ((i<distr_size)&&(i>=0)) {
        return prob_M1[i];
    }
    else
...
double distribution::get_prob_disp (long i) const {
    if ((i<distr_size)&&(i>=0)) {
...
double distribution::get_entropy () const {
...
double distribution::extreme_part_inside (double sigma_in) const {
...
void distribution::resize_arrays () {
...
void distribution::shift_arrays (long from, long to) {
...
void distribution::init_interval (double x, long i) {
    reinit_interval (x, i);
    prob_filled++;
}

void distribution::reinit_interval (double x, long i) {
    global_width += 2.0e-6;
    prob_histogram[i] = 1;
...
bool distribution::add_interval (double x) {
...
void distribution::add_val_to_interval (double x, long i) {
    prob_histogram[i]++;
    if (x<prob_mins[i]) {
...
void distribution::merge_and_init (double x, long nearest_h_left_i) {
