...
class minima : public extrema {
public:
    minima () { limit_value = 1.0e12; }
    ~minima() = default;
...
void minima::step (double new_value) {
    if (initialized) {
...
void minima::get (bool &result, double &time, double &value) {
    result = false;
    if (lasttimeisfound) {
        value = val[marker];
