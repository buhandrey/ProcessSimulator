...
std::string gasdev::get_basename () {
    std::stringstream name_temp;
    name_temp << "_" << modelname << "_s_" << par_sigma << "_m_" << par_mu << "_idum_" << idum;
    return name_temp.str();
}

void gasdev::set_pars (int set_size, double set_par_mu, double set_par_sigma, long set_idum) {
    if (set_size>0) {
        val.resize(set_size);
    }
    else {
