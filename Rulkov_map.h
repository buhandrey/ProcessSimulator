...
std::string Rulkov_map::get_basename () {
    std::stringstream name_temp;
    name_temp << "_" << modelname << "_x0_" << x0 << "_y0_" << y0 << "_a_" << par_a << "_m_" << par_m << "_s_" << par_s;
    return name_temp.str();
}

void Rulkov_map::set_par_a (double set_par) {
    par_a = set_par;
    par_a_def = true;
    try_init ();
}

void Rulkov_map::set_par_m (double set_par) {
    par_m = set_par;
    par_m_def = true;
    try_init ();
}

void Rulkov_map::set_par_s (double set_par) {
    par_s = set_par;
    par_s_def = true;
    try_init ();
}

void Rulkov_map::set_buffer_size (long set_size) {
    if (calc_lyap)
...
void Rulkov_map::set_x0 (double set_var0) {
...
void Rulkov_map::set_y0 (double set_var0) {
    if (!par_buffersize_def)
...
void Rulkov_map::function (long kin, double portion, long kout) {
